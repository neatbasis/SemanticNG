...
- `coverage.xml` is generated by running `make test-cov` (which calls `pytest --cov --cov-report=term-missing --cov-report=xml`) or by invoking the same `pytest` command directly.
- In CI, the canonical expectation is a repository-root artifact at `coverage.xml` produced by the coverage job and attached/published with the run.
- Review ownership:
  - During PR review: the PR author and assigned reviewer(s) confirm the `coverage.xml` artifact and reported percentages are present and consistent with the diff scope.
  - During sprint close/release readiness: the release/governance owner confirms coverage evidence appears in sprint-close artifacts and aligns with threshold policy.
- Failure handling:
  - **Blocking**: missing `coverage.xml` in CI for a coverage-required PR/sprint-close package, failing configured threshold (`[tool.coverage.report].fail_under`), or unexplained negative threshold delta versus approved baseline.
  - **Advisory**: non-threshold-impacting shifts in changed-module coverage that remain above threshold and include rationale/mitigation in handoff notes.
