...
- 2026-02-28 (Now): Promoted **Halt normalization contract** from `Next/proven` milestone placement to `Now/proven`; rationale: `gate_halt_unification` acceptance command is green (`82 passed, 4 skipped`) and the halt path is no longer a forward milestone risk.
