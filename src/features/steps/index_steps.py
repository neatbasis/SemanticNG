...
def step_then_proposal_has_schema_and_extraction(context):
    ...
    assert "extraction" in proposal and isinstance(proposal["extraction"], dict), "proposal.extraction missing/invalid."
...
def step_then_proposal_has_evidence_spans(context):
    ...
    for key in ["name", "startDate"]:
        span = spans.get(key, {})
        assert span.get("start", -1) >= 0 and span.get("end", -1) >= 0, f"Invalid evidence span for {key}"
...
def step_then_proposed_schema_is_event_v1(context):
    proposal = getattr(context, "_last_proposal", None)
    assert proposal is not None, "No proposal found."
    assert proposal.get("schema_id") == "Event.v1", f"Expected Event.v1, got {proposal.get('schema_id')}"
...
def step_then_artifact_persisted(context):
    ...
    assert art.identifier.startswith("art:"), f"Expected artifact id to start with art:, got {art.identifier}"
...
def step_then_artifact_links_source(context):
    ...
    assert source_id == context._last_ingested_resource.identifier, "Artifact source_identifier does not match ingested resource."
...
def step_then_search_hack_night_returns_artifact(context):
    ...
    assert art.identifier in results, f"Artifact not found in top results for 'hack night'. Got: {results}"
...
def step_then_search_maria01_returns_artifact(context):
    ...
    assert art.identifier in results, f"Artifact not found in top results for 'Maria 01'. Got: {results}"
...
def step_then_schema_usage_updated(context):
    ...
    assert usage["Event.v1"]["successful_validations"] >= 1, "successful_validations not incremented"
