...
def test_projection_replay_is_deterministic_after_restart_with_same_log(
    ...
    prediction_log = tmp_path / "predictions.jsonl"
    initial_projection = ProjectionState(
    ...
    assert (
        recovered_after_restart.current_predictions.keys()
        == online_after_turn_1.current_predictions.keys()
    )
    ...
    assert (
        replay_after_turn_2.projection_state.current_predictions.keys()
        == online_after_turn_2.current_predictions.keys()
    )
    assert (
        replay_after_turn_2.projection_state.correction_metrics
        == online_after_turn_2.correction_metrics
    )
...
def test_halt_artifacts_keep_explainability_fields_when_projection_is_replayed(
    ...
    halt_artifacts = [
        a
        for a in persisted_episode["artifacts"]
        if a.get("artifact_kind") in {"authorization_issue", "halt_observation"}
    ]
...
def test_replay_analytics_snapshot_matches_across_consumer_paths_for_same_log(
    tmp_path: Path,
) -> None:
    prediction_log = tmp_path / "predictions.jsonl"
...
def test_replay_projection_snapshot_remains_identical_for_identical_logs_with_human_requests(
    tmp_path: Path,
) -> None:
